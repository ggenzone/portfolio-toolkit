            ...
            if transaction["ticker"] is not None:  # Only for real assets, not cash
                ...
                if transaction["type"] == "buy":
                    ...
                    cash_transaction = {
                        ...
                        "type": "sell",
                        "quantity": transaction.get("total_base", transaction.get("total", 0)),
                        "price": 1.00,
                        ...
                        "fees_base": transaction.get("fees_base", 0),  # Fees already deducted
                        "total_base": transaction.get("total_base",0)
                    }
                elif transaction["type"] == "sell":
                    # When selling assets, cash increases (buy cash)
                    net_proceeds = transaction.get("total_base", transaction.get("total", 0)) - transaction.get("fees_base", 0)
                    cash_transaction = {
                        "date": transaction["date"],
                        "type": "buy",
