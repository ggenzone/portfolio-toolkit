...
def preprocess_data(assets, start_date, data_provider):
    ...
    for ticker, prices in historical_prices.items():
        ...
        for date in dates:
            ...
            current_market_value_base = convert_price_to_base_currency(ticker, price, assets)
...
def calculate_cost(date, transactions):
    ...
    current_quantity = 0
    total_cost = 0
    fifo = []  # List to handle purchases (FIFO): (quantity, cost_per_unit_in_base_currency)
    ...
    for transaction in transactions:
        ...
        if transaction_date <= date:
            if transaction["type"] == "buy" or transaction["type"] == "deposit":
                ...
                total_cost_base = transaction.get("total_base", transaction["quantity"] * transaction["price"])
                cost_per_unit = total_cost_base / transaction["quantity"] if transaction["quantity"] > 0 else 0
