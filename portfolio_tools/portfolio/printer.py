def print_current_positions(df_portfolio, target_date=None):
    ...
    if target_date:
        ...
        try:
            ...
            if target_datetime.date() not in available_dates:
                ...
                earlier_dates = [
                    d for d in available_dates if d <= target_datetime.date()
                ]
                ...
                print(
                    f"No data for {target_date}, using closest earlier date: {closest_date}"
                )
        ...
        tickers_with_positions = set(
            df_portfolio[df_portfolio["Quantity"] > 0]["Ticker"].unique()
        )
    ...
    summary["Return (%)"] = (
        (summary["Value_Base"] - summary["Cost"]) / summary["Cost"]
    ) * 100
    summary["Return (%)"] = (
        summary["Return (%)"].replace([float("inf"), float("-inf")], 0).fillna(0)
    )
    ...
    total_return = (
        ((total_value_base - total_cost) / total_cost) * 100 if total_cost > 0 else 0
    )

    print(f"Current positions as of {last_date.date()}:")
    print(
        f"{'| Ticker':<10}| {'Price Base':<12}| {'Cost':<12}| {'Quantity':<10}| {'Value Base':<12}| {'Return (%)':<12}|"
    )
    print(
        "|"
        + "-" * 9
    ...
    for _, row in summary.iterrows():
        ...
        if row["Ticker"].startswith("__"):
            ...
            display_price = (
                row["Value_Base"] / row["Quantity"] if row["Quantity"] > 0 else 0
            )

        print(
            f"| {row['Ticker']:<8}| {display_price:<11.2f}| {row['Cost']:<11.2f}| {row['Quantity']:<8.2f}| {row['Value_Base']:<10.2f}| {row['Return (%)']:<10.2f}|"
        )
    ...
    print(
        f"| {'TOTAL':<8}| {'':<11}| {total_cost:<11.2f}| {'':<8}| {total_value_base:<10.2f}| {total_return:<10.2f}|"
    )
