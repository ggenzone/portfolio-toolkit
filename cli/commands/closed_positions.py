...
@click.command(name='closed-positions')
...
def closed_positions(portfolio_file, date):
    """Print closed portfolio positions as a table."""
    data_provider = YFDataProvider()
    portfolio = Portfolio(json_filepath=portfolio_file, data_provider=data_provider)
    portfolio.print_closed_positions(date)
